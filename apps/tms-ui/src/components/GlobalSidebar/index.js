...
const Sidebar = ({
  ...
  const handleOpenChange = _openKeys => {
    isMainMenu = key => menus.some(item => key && (item.key === key || item.path === key));

    const lastOpenKey = _openKeys[_openKeys.length - 1];
    const moreThanOne = _openKeys.filter(openKey => isMainMenu(openKey)).length > 1;

    setOpenKeys(moreThanOne ? [lastOpenKey] : [..._openKeys]);
  };


  // Get the currently selected menu
  const getMenuMatchKeys = (flatMenuKeys, path) => flatMenuKeys.filter(item => pathToRegexp(item).test(path));
  getSelectedMenuKeys = () => {
  ...
  // permission to check
  var checkPermissionItem = (authority, ItemDom) => {
    if (this.props.Authorized && this.props.Authorized.check) {
      const { check } = this.props.Authorized;
      return check(authority, ItemDom);
    }
